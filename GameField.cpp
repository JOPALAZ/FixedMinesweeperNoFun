...
bool GameField::MakeGameField(std::filesystem::path path, int difficulty) {
    ...
    if (WINDOW_RES.first > WINDOW_RES.second) {
        minefieldScaleX = 1.f;
        minefieldScaleY = WINDOW_RES.second*(1.f-MARGIN_SCALE) / (float)WINDOW_RES.first;
    }
    else {
        minefieldScaleY = 1.f;  
        minefieldScaleX = WINDOW_RES.first / (float)WINDOW_RES.second/ (1.f - MARGIN_SCALE);
    }
    ...
    float scale = std::min((WINDOW_RES.first * 0.32f) / bombAmount.getLocalBounds().width, (WINDOW_RES.first * MARGIN_SCALE * 0.8f) / bombAmount.getLocalBounds().height);
    ...
    try {
        ...
        this->isOK = false;
        std::cout << "EXCEPTION: " << ex.what(); return false;
    }
    gameIsOn = true;
    return true;
}
